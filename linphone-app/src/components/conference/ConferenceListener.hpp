...
// LINPHONE LISTENERS
	...
	virtual void onParticipantDeviceIsScreenSharingChanged(const std::shared_ptr<linphone::Conference> & conference, const std::shared_ptr<const linphone::ParticipantDevice> & device, bool enabled) override;
...
signals:
	...
	void participantDeviceIsScreenSharingChanged( const std::shared_ptr<const linphone::ParticipantDevice> & device, bool enabled);
